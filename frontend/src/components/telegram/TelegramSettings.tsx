...
const TelegramSettings: React.FC = () => {
    ...
    const fetchAvailableChats = async () => {
        ...
        try {
            ...
            toast({ title: "Error", description: "Could not fetch available chats.", variant: 'destructive' });
        } finally {
            setRefreshing(false);
        }
    };
    ...
    const handleSaveFromDashboard = async (updatedChats: TelegramChat[]): Promise<boolean> => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                toast({ title: "Settings Saved!", description: "Your Telegram monitoring settings have been updated." });
